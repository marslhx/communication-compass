...
export function About(){
  ...
  return <Page title="关于本站"><div className="about">
    ...
    <div className="about-grid"><section><h2>作者与联系</h2><dl><dt>作者／项目负责人</dt><dd><a className="author-link" href={siteConfig.authorUrl} target="_blank" rel="noreferrer"><strong>{siteConfig.author}</strong><span aria-hidden="true">↗</span></a>{siteConfig.affiliation&&` · ${siteConfig.affiliation}`}</dd><dt>联系邮箱</dt><dd><a href={siteConfig.contactUrl}>{siteConfig.contactLabel}</a></dd></dl></section><section><h2>版权、引用与隐私</h2><ul><li>本站原创文字与数据：{siteConfig.contentLicense}。</li><li>程序代码：{siteConfig.codeLicense}。</li><li>第三方引文、肖像与图表按各自来源与授权使用；理论与事实性介绍不替代原始文献。</li><li>引用本站时，请标注页面名称、COMMPASS｜传播学指南针 Beta、访问日期与页面地址。</li></ul><p>当前版本不设账户、不使用站内行为分析；浏览器只在本地保存深浅色偏好和未完成的反馈草稿。提交反馈时将前往 GitHub Discussions，其数据处理适用 GitHub 的相关规则。</p></section></div>
  </div></Page>
}
